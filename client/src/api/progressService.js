 ...
 * @returns {Promise<object>} - The response data from the server, including score and points earned.
 */
const submitQuizResult = async (submissionData, token) => {
  ...
  try {
    ...
    throw new Error(error.response?.data?.message || 'Could not submit quiz results.');
  }
};
